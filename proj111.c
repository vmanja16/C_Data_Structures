  ...
  if (argc > 2)  // MODE 1
    {
      ...
      //FEL will be int array with 2 arguments: event (0 arrival,1 arrival,-1 
departure, 
      //2 executed) and time
      ...
      while( (TasksRemain0 > 0) || (TasksRemain1 > 0) )
	{
	  ...
	  //At this point Current_FEL contains all the time events, there are 
NumberOfTimes
...
GetServiceTime(AvgServiceTime);
	...
	// CUMULATIVE STATISTICS
      ...
      printf("%f\t%f\t%f\t%f\t\n", AverageQueLength, AverageWatitingTime0, 
AverageWatitingTime1, AverageCPU);
