...
/*-----------------------------Check queuelist for priority 1------------------------------------*/
 ...
 NextNode = CurrentNode->next;
      if (NextNode != NULL)
        {
          if (NextNode->Time == CurrentNode->Time) { CurrentNode = CurrentNode->next;  if (CurrentNode->priority == 0) {QueueLength0++; Previous_Queue0 = QueueLength0;} if 
(CurrentNode->priority == 1){QueueLength1++; Previous_Queue1 = QueueLength1;} 
...
//printf(" \nTime = %d\n\n", time);
  ...
  double AverageWatitingTime0 = 0;
   ...
   fprintf(fp3, "avgWaitTime0: %lf\navgWaitTime1: %lf\navgQLen: %lf\navgCPUUtil: %lf\navgBalance: %lf\n", AverageWatitingTime0, AverageWatitingTime1, AverageQueLength, 
AverageUtil, Average_Load_Balance);
